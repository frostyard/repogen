...
func TestIncrementalModeCopiesNewPackages(t *testing.T) {
	...
	tmpDir, err := os.MkdirTemp("", "repogen-test-pacman-incremental-")
	if err != nil {
		t.Fatalf("Failed to create temp dir: %v", err)
	}
	defer func() { _ = os.RemoveAll(tmpDir) }()
	...
	pkgAPath := filepath.Join(outputDir, "x86_64", "pkga-1.0-1-x86_64.pkg.tar.zst")
	if _, err := os.Stat(pkgAPath); os.IsNotExist(err) {
		t.Fatalf("Package A was not copied: %v", err)
	}

	// Step 2: Simulate S3 sync - keep only database files, remove package files
	...
	pkgBPath := filepath.Join(outputDir, "x86_64", "pkgb-1.0-1-x86_64.pkg.tar.zst")
	...
	} else if string(pkgBContent) != "fake pacman package B" {
		t.Errorf("Package B has wrong content: %s", pkgBContent)
	}

	// Step 7: Verify database was regenerated successfully
	dbPath := filepath.Join(outputDir, "x86_64", "test-repo.db.tar.zst")
	if _, err := os.Stat(dbPath); os.IsNotExist(err) {
		t.Errorf("Database not created: %s", dbPath)
	}
